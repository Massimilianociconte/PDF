...
from app.utils.security import validate_safe_filename, validate_uuid
...
async def download_converted_image(
    ...
    safe_output_id = validate_uuid(output_id)
    safe_filename = validate_safe_filename(filename)

    file_path = os.path.join(settings.upload_dir, safe_output_id, safe_filename)
