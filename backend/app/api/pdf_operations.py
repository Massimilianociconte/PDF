...
async def upload_pdf(
    ...
    # Save file
    # Optimize: Stream file to disk to avoid loading large files into memory
    file_size = 0
    try:
...
async def add_image_to_pdf(
    ...
    async with aiofiles.open(image_path, "wb") as out_file:
        while content := await image.read(1024 * 1024):  # 1MB chunk
            await out_file.write(content)
...
async def sign_pdf(
    ...
    async with aiofiles.open(sig_path, "wb") as out_file:
        while content := await signature.read(1024 * 1024):  # 1MB chunk
            await out_file.write(content)
