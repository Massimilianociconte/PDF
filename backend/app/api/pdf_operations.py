    ...
    async with aiofiles.open(file_path, "wb") as f:
        while content := await file.read(1024 * 1024):  # 1MB chunks
            ...
            if file_size > settings.max_file_size:
                # Close the file before removing it (handled by async with context exit but let's be safe if we raise)
                pass
            await f.write(content)

    # Check size after upload (or during if we want to abort early,
    # but cleaning up partial file requires handling context exit properly)
    if file_size > settings.max_file_size:
        os.remove(file_path)
...
async def sign_pdf(
    ...
    async with aiofiles.open(sig_path, "wb") as f:
        while content := await signature.read(1024 * 1024):
            await f.write(content)
    ...
    try:
        pdf_service.sign_pdf(file_path, output_path, sig_path, page, x, y, width, height)
    except Exception as e:
