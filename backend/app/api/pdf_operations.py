...
async def upload_pdf(
    ...
    # Blocking I/O in threadpool for file writing
    await run_in_threadpool(lambda: open(file_path, "wb").write(content))

    # Get PDF info (Blocking CPU)
    try:
...
def get_pdf_info(
    ...
    # This runs in threadpool because endpoint is 'def'
...
async def sign_pdf(
    ...
    await run_in_threadpool(lambda: open(sig_path, "wb").write(content))
    ...
    try:
        ...
        if os.path.exists(sig_path):
            await run_in_threadpool(os.remove, sig_path)
